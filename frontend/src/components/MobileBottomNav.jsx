...
export const MobileBottomNav = ({ activeTab, onNavigate, onOpenPostModal, savedCount = 0 }) => {
  const handleDashboardClick = () => {
    try {
      ...
      if (stored) {
        ...
        const userRole = String(parsed.role || parsed.user_role || '').toUpperCase();
        if (userRole === 'ADMIN') {
          onNavigate('admin');
          return;
        }
  ...
  return (
    <div className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-md border-t border-gray-200 py-1.5 px-3 flex justify-around items-center shadow-lg font-bengali">
      {/* Home */}
      <button
      ...
      <button
        ...
        className={`flex flex-col items-center gap-0.5 text-[10px] font-medium transition cursor-pointer ${
          activeTab === 'admin' || activeTab === 'dashboard' ? 'text-[#168A45]' : 'text-gray-500'
        }`}
      >
